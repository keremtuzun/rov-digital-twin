...
| SubPipe | <https://zenodo.org/records/12666132> | High | Real RGB, side-scan sonar, navigation/sensor data, annotations | Pipeline objects/regions; not a direct nine-class condition taxonomy | Zenodo API metadata for record 12666132 identifies version 3.0.1 as open and CC BY 4.0 | No until internal approval | Snapshot the record/API license evidence; retain attribution/version/DOI; verify per-file coverage; hash assets; approve manifest rows | Use independent RGB frames for structure/domain robustness; keep sonar/navigation outside the current RGB evaluator |
...
| SubPipe | CC BY 4.0 in the official Zenodo API metadata | Yes | Yes under CC BY terms | Yes | Yes | No separate owner permission indicated; **internal per-asset approval is still required** |
