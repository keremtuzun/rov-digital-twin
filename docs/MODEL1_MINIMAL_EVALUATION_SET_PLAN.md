...
| SubPipe | <https://zenodo.org/records/12666132> | High | Real RGB, side-scan sonar, navigation/sensor data, annotations | Pipeline objects/regions; not a direct nine-class condition taxonomy | Version 3.0.1 confirmed, but reviewed official API rights field is null; written license scope required | No | Obtain authorized license/file-scope confirmation; retain attribution/version/DOI; verify per-file coverage; hash assets; approve manifest rows | Use independent RGB frames for structure/domain robustness only after approval; keep sonar/navigation outside the current RGB evaluator |
...
| CleanCam | <https://zenodo.org/records/21515620> | Medium | Real and synthetic underwater RGB | viewport fouling/cleanliness levels; visibility-like degradation | Current v2.0.0 confirmed, but reviewed official API rights field is null; written license scope required | No | Obtain authorized version/license confirmation, retain attribution, separate real from synthetic, hash assets, and approve manifest rows | Candidate for camera fouling/visibility robustness after approval; labels require manual mapping because viewport fouling is not structural biofouling |
...
No candidate is marked usable now because the repository's own governance gate requires an authoritative license or grant, approved reviewer, asset URL, download timestamp, SHA-256, license URL, and attribution for every accepted asset. CleanCam's current record also requires explicit license clarification before per-asset review.
...
| SubPipe | Official API rights field reviewed as null | Unresolved | Unresolved | Public files shown | Citation wording shown | **Yes: authorized license/file-scope clarification required** |
...
| CleanCam | Current v2.0.0 official API rights field reviewed as null | Unresolved | Unresolved | Public file shown | Citation metadata shown | **Yes: authorized license/version/file-scope clarification required** |
