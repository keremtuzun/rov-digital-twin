...
| InspectVQA | <https://huggingface.co/datasets/anonymousSubmissionVqa2026/InspectVQA> | Hugging Face Community | Yes | `DO_NOT_SEND_USER_DIRECTED` | CC BY-NC/owner authority unresolved | Prepared draft only; not sent | No | No | Keep excluded; use open-license sources instead |
...
| Structural Defects — Bonnín-Pascual / Ortiz | <https://xiscobonnin.github.io/resources/> | Published UIB/SRV contact | Yes | `DO_NOT_SEND_USER_DIRECTED` | Explicit image/mask reuse license not recorded | Prepared draft only; not sent | No | No | Keep excluded under open-license-only strategy |
| WPI / ARL Corrosion | <https://arl.wpi.edu/corrosion_dataset/> | Published dataset contact | Yes | `DO_NOT_SEND_USER_DIRECTED` | Exact dataset/output license scope unresolved | Prepared draft only; not sent | No | No | Keep excluded; use open-license sources instead |
...
| SeaClear Marine Debris | <https://data.4tu.nl/datasets/4f1dff25-e157-4399-a5d4-478055461689/1> | 4TU.ResearchData / named creators | No separate request required | `SOURCE_MANIFEST_BUILT_REVIEW_PENDING` | CC BY 4.0 verified on canonical record and DataCite metadata | License evidence plus 8,610-row hashed staging manifest under `data/model1_baseline_v2/` | No, pending human review/canonical split/approval | No, pending human review/canonical split/approval | Perform independent image review; source proposals are not approvals; obtain missing schema coverage separately |
...
1. Do not send InspectVQA, Structural Defects, or WPI/ARL requests; proceed with the user-directed open-license path.
2. Review the completed SeaClear staging manifest under the independent annotation instructions; preserve site/camera grouping and exact attribution.
3. Monitor the sent SubPipe and CleanCam issues; archive complete responses and follow up on `2026-09-02 17:00 Europe/Istanbul` if neither has responded.
4. Send the Claru inquiry only after a human confirms budget and acceptable contract constraints.
5. Send the K-Pipelines provenance question if synthetic supplemental testing remains useful.
...
The SubPipe and CleanCam requests above were sent and their public GitHub receipts are recorded. No approval was inferred from sending or public availability. SeaClear v1 was downloaded only after its canonical 4TU record and CC BY 4.0 license were verified; its publisher MD5 matches and its raw assets remain Git-ignored. No labels, metrics, checkpoints, provider responses, license grants, or approvals were fabricated. No Model 1 training was run. No Model 2 or Twin 2 work was performed. Original Model 1 remains blocked/not frozen, and `model1_baseline_v2` remains an inactive fallback until its activation gate is met.
