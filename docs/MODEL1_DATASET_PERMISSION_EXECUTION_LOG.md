...
| InspectVQA | <https://huggingface.co/datasets/anonymousSubmissionVqa2026/InspectVQA> | <https://huggingface.co/datasets/anonymousSubmissionVqa2026/InspectVQA/discussions/new>, then identified licensor | Yes | `READY_FOR_AUTHENTICATED_SEND` | Dataset card states CC BY-NC 4.0 or owner-approved alternative; licensor authority and competition/output rights unresolved | Verified route/message; no send receipt or approval | No | No | Reconnect authenticated browser, post request, and obtain an identifiable owner response covering images, annotations, derivatives, outputs, and restrictions |
...
| Structural Defects — Bonnín-Pascual / Ortiz | <https://xiscobonnin.github.io/resources/> | `xisco.bonnin@uib.es`, published UIB/SRV contact | Yes | `READY_FOR_AUTHENTICATED_SEND` | Download and citation are published; explicit image/mask reuse license not recorded | Verified contact/message; no send receipt or approval | No; non-underwater supplemental source only | No | Reconnect authenticated email, send prepared request, and archive written rights response |
| WPI / ARL Corrosion | <https://arl.wpi.edu/corrosion_dataset/> | `gr-wpi-arl-dsrg@wpi.edu`, published dataset contact | Yes | `READY_FOR_AUTHENTICATED_SEND` | Access/citation information reported; explicit dataset license and output rights unresolved | Verified contact/message; no send receipt or approval | No; laboratory/non-underwater supplemental source only | No | Reconnect authenticated email, send prepared request, and archive explicit terms and covered version/file list |
...
1. Reconnect the authenticated Chrome browser and send the ready InspectVQA, Structural Defects, and WPI/ARL requests after grouped action-time confirmation.
